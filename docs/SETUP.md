...
Explorer 内で対象を右クリックし、`Insert from File…`（ファイルから挿入）で選ぶ。
...
似た名前が3つ並んでいる。末尾に `…` が付いているものがファイル選択ダイアログを開く。

| メニュー | 動作 | 今回 |
|---|---|---|
| パーツを挿入 / Insert Part | Part を1個置く | 使わない |
| オブジェクトを挿入 / Insert Object | 種類を選んで新規作成する | 使わない |
| **ファイルから挿入… / Insert from File…** | **保存済みファイルを読み込む** | **これ** |
